...
function TeamsCategoryUpdatePopUp({ teamsCategory, closePopUp }) {
  const [name, setName] = useState({
    value: teamsCategory.name,
    error: false,
  });
  const [error, setError] = useState({ message: "", error: false });
  const [success, setSuccess] = useState({ message: "", success: false });

  const { updateTeamsCategory } = useContext(TeamsCategoriesContext);
  ...
  const handleSubmit = (e) => {
    ...
    if (name.error) {
      ...
      updateTeamsCategory(body, teamsCategory._id, setError, setSuccess);
    }
  };
...
export default TeamsCategoryUpdatePopUp;
