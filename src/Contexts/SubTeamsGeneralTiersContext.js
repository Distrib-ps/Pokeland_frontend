...
const SubTeamsGeneralTiersProvider = ({ children }) => {
    ...
    async (errorCallback, teamsGeneralTierId) => {
      try {
        const responseServer = await axios.get(
          `http://localhost:8000/sub-teams-general-tiers/tiers/${teamsGeneralTierId}`,
          {
            headers: { token: token },
          }
        );
    ...
    async (body, errorCallback, successCallback) => {
      try {
        const response = await axios.post(
          "http://localhost:8000/sub-teams-general-tiers/add",
          body,
          {
            headers: { token: token },
          }
    ...
    async (body, id, errorCallback, successCallback) => {
      try {
        const response = await axios.put(
          `http://localhost:8000/sub-teams-general-tiers/update/${id}`,
          body,
          {
            headers: { token: token },
          }
  ...
  const deleteSubTeamsGeneralTier = useCallback(
    async (id, errorCallback, successCallback) => {
      try {
        await axios.delete(
          `http://localhost:8000/sub-teams-general-tiers/delete/${id}`,
          {
            headers: { token: token },
          }
        );
